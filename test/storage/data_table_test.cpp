...
public:
  ...
  RandomDataTableTestObject(storage::BlockStore &store, uint32_t max_col,
                            double null_bias, Random &generator)
  ...
  template <typename Random>
  bool RandomUpdateTuple(const timestamp_t timestamp,
                         const storage::TupleSlot &slot, Random &generator) {
    ...
    auto *undo = storage::DeltaRecord::InitializeDeltaRecord(
        undo_buffer, timestamp, layout_, update_col_ids);
...
private:
  ...
  std::unordered_map<storage::TupleSlot, std::vector<tuple_version>>
      tuple_versions_;
...
TEST_F(DataTableTests, SimpleInsertSelect) {
  ...
  for (uint32_t i = 0; i < repeat; i++) {
    RandomDataTableTestObject tested(block_store_, max_col,
                                     null_ratio_(generator_), generator_);
    ...
    for (auto slot : inserted_slots_) {
      ...
      EXPECT_TRUE(testutil::ProjectionListEqual(
          tested.Layout(), *select_row, *tested.GetInsertedRow(slot, 0)));
    }
  }
}
...
TEST_F(DataTableTests, SimpleVersionChain) {
  ...
  for (uint32_t i = 0; i < repeat; i++) {
    ...
    for (uint32_t j = 0; j < num_updates; j++) {
      ...
      EXPECT_TRUE(testutil::ProjectionListEqual(
          tested.Layout(), *select_row, *tested.GetInsertedRow(slot, j)));
    }
  }
}
...
TEST_F(DataTableTests, WriteWriteConfilictUpdateFails) {
  ...
  for (uint32_t i = 0; i < repeat; i++) {
    ...
    auto slot = tested.InsertRandomTuple(generator_);
    EXPECT_TRUE(
        tested.RandomUpdateTuple(timestamp_t(UINT64_MAX), slot, generator_));
